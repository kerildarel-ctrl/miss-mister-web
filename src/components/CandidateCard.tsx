...
export const CandidateCard: React.FC<CandidateCardProps> = ({
  ...
  const getRankBadgeClass = (rank: number) => {
    if (rank === 1) return 'gold-gradient-btn text-slate-950 font-black shadow-lg ring-2 ring-amber-400/80';
    if (rank === 2) return 'blue-pill-btn text-white font-black shadow-md';
    if (rank === 3) return 'bg-purple-600/80 text-white font-black backdrop-blur-md border border-white/30';
  ...
  return (
    <motion.div
      ...
      className={`group relative rounded-[2rem] glass-mirror-panel overflow-hidden flex flex-col justify-between transition-all duration-400 font-poppins p-3.5 ${
        candidate.rank === 1 ? 'ring-2 ring-amber-400/60 shadow-amber-500/30' : ''
      }`}
    >
      {/* Top Image Container */}
      <div className="relative h-72 sm:h-80 w-full overflow-hidden rounded-[1.5rem] bg-slate-950">
        {!imageError && candidate.photoUrl ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-amber-600/80 via-cyan-600/50 to-slate-950 flex flex-col items-center justify-center p-6 text-white text-center">
            <div className="w-20 h-20 rounded-full bg-white/20 backdrop-blur-xl flex items-center justify-center font-black text-3xl mb-2 border border-white/40 shadow-lg">
              {getInitials()}
            </div>
        ...
        <div className="absolute top-3.5 left-3.5 bg-slate-950/80 backdrop-blur-xl px-3.5 py-1.5 rounded-2xl border border-white/40 font-mono font-black text-xs text-amber-400 shadow-xl flex items-center gap-1.5 z-10">
        ...
        <div className="absolute top-3.5 right-3.5 z-10">
          <span className={`px-3.5 py-1.5 rounded-2xl text-xs flex items-center gap-1.5 shadow-xl ${getRankBadgeClass(candidate.rank)}`}>
        ...
        <div className="absolute bottom-3.5 left-3.5 right-3.5 flex items-center justify-between z-10">
          <span className="px-3.5 py-1 rounded-xl bg-slate-950/80 backdrop-blur-xl text-[10px] uppercase font-black text-amber-300 tracking-wider border border-white/30">
            {candidate.category}
          </span>
        </div>
      ...
      {/* ULTRA TRANSPARENT FROSTED GLASS INNER BOX */}
      <div className="mt-3.5 p-5 space-y-4 flex-1 flex flex-col justify-between glass-inner-box text-white">
        <div>
          <Link
            href={candidateUrl}
        ...
        {/* Vote Score Glass Inner Track */}
        <div className="space-y-2 bg-slate-950/60 backdrop-blur-md p-3.5 rounded-2xl border border-white/20">
          ...
          {/* Progress track */}
          <div className="w-full h-2.5 rounded-full bg-slate-900 overflow-hidden p-0.5 border border-white/20">
            <div
        ...
        <div className="pt-1">
          <button
            ...
            className="w-full gold-gradient-btn py-3.5 px-4 rounded-2xl font-black text-xs text-slate-950 shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-transform uppercase tracking-wider"
          >
