...
export default function DirectCandidatePage({
  ...
  const rawSlug = resolvedParams?.candidateSlug || '';
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // Find candidate by slug format (firstName-lastName or id)
      const foundCand = cands.find((c) => {
        const slugFormat = `${c.firstName}-${c.lastName}`.toLowerCase().replace(/[^a-z0-9]+/g, '-');
        const numberFormat = c.candidateNumber.replace('#', '').toLowerCase();
        return (
          slugFormat === rawSlug.toLowerCase() ||
          c.id === rawSlug ||
          numberFormat === rawSlug.toLowerCase()
        );
      });
    ...
    return (
      <>
        ...
        <main className="min-h-screen bg-[#F8FAFC] py-20 text-center font-poppins space-y-4">
          ...
          <p className="text-sm text-slate-500">Le profil demandé n&apos;existe pas ou a été déplacé.</p>
  ...
  return (
    <>
      ...
      <main className="flex-1 min-h-screen py-10 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto space-y-8 font-poppins bg-[#F8FAFC]">
        ...
        >
          <ArrowLeft className="w-4 h-4 text-blue-600" /> Retour à {competition?.title || 'la compétition'}
        </Link>
